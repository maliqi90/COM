...

{
	...
	*/
	...
	//_stprintf(szPort, MAX_PATH, _T("\\\\.\\COM%d"), portnr);///可以显示COM10以上端口//add by itas109 2014-01-09
	...
	_stprintf(szBaud,_T("baud=%d parity=%c data=%d stop=%d"), baud, parity, databits, mystop);
	...
	*/
	...
	if (m_hComm == INVALID_HANDLE_VALUE)
	{
		...
		case ERROR_FILE_NOT_FOUND:
		{
									 ...
									 _stprintf(Temp,  _T("COM%d ERROR_FILE_NOT_FOUND,Error Code:%d"), portnr, GetLastError());
		...
		{
									TCHAR Temp[200] = { 0 };
									_stprintf(Temp, _T("COM%d ERROR_ACCESS_DENIED,Error Code:%d"), portnr, GetLastError());
									MessageBox(NULL, Temp, _T("COM InitPort Error"), MB_ICONERROR);
									break;
		}
		default:
		...
		*/
		...
		if (!bResult)
		{
			...
			case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
			{
										 ...
										 TCHAR Temp[200] = { 0 };
                                        _stprintf(Temp,_T("COM%d ERROR_ACCESS_DENIED,WaitCommEvent() Error Code:%d"), port->m_nPortNr, GetLastError());
			...
			case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
			{
									   port->m_hComm = INVALID_HANDLE_VALUE;
									   TCHAR Temp[200] = { 0 };
									   _stprintf(Temp,_T("COM%d ERROR_BAD_COMMAND,WaitCommEvent() Error Code:%d"), port->m_nPortNr, GetLastError());
									   MessageBox(NULL, Temp, _T("COM WaitCommEvent Error"), MB_ICONERROR);
									   break;
			}
		...
		if (bWrite)
		{
			...
			if (!bResult)
			{
				...
				case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
											 port->m_hComm = INVALID_HANDLE_VALUE;
											 TCHAR Temp[200] = { 0 };
											 _stprintf(Temp,_T("COM%d ERROR_ACCESS_DENIED,WriteFile() Error Code:%d"), port->m_nPortNr, GetLastError());
				...
				case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
										   port->m_hComm = INVALID_HANDLE_VALUE;
										   TCHAR Temp[200] = { 0 };
										   _stprintf(Temp,_T("COM%d ERROR_BAD_COMMAND,WriteFile() Error Code:%d"), port->m_nPortNr, GetLastError());
										   MessageBox(NULL, Temp, _T("COM WriteFile Error"), MB_ICONERROR);
										   break;
				}
				default:
		...
		if (bRead)
		{
			...
			if (!bResult)
			{
				...
				case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
											 port->m_hComm = INVALID_HANDLE_VALUE;
											 TCHAR Temp[200] = { 0 };
											 _stprintf(Temp,_T("COM%d ERROR_ACCESS_DENIED,ReadFile() Error Code:%d"), port->m_nPortNr, GetLastError());
				...
				case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
										   port->m_hComm = INVALID_HANDLE_VALUE;
										   TCHAR Temp[200] = { 0 };
										   _stprintf(Temp,_T("COM%d ERROR_BAD_COMMAND,ReadFile() Error Code:%d"), port->m_nPortNr, GetLastError());
										   MessageBox(NULL, Temp, _T("COM ReadFile Error"), MB_ICONERROR);
										   break;
				}
				default:
